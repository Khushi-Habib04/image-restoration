...
class KBNet_s(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=12, enc_blk_nums=[2, 2, 4, 8],
                 dec_blk_nums=[2, 2, 2, 2], basicblock='KBBlock_s',basicblock2='KBBlock_ss', lightweight=False, ffn_scale=2):
        super().__init__()
        ...
        self.middle_blks = \
            nn.Sequential(
                *[basicblock2(chan, FFN_Expand=ffn_scale) for _ in range(middle_blk_num)]
            )
        ...
        for num in dec_blk_nums:
            ...
            self.decoders.append(
                nn.Sequential(
                    *[basicblock2(chan, FFN_Expand=ffn_scale) for _ in range(num)]
                )
            )
