...
class KBBlock_s(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, nset=32, k=3, gc=4, lightweight=False, GCE_Conv=2):
        ...
        self.conv02 = nn.Conv2d(in_channels=dw_ch, out_channels=dw_ch,
                               ...
                               bias=True)
        self.depthwise_conv31 = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=5, padding=4, stride=1, dilation=2, groups=c, bias=True)
        self.pointwise_conv31 = nn.Conv2d(in_channels=c, out_channels=c//2, kernel_size=1, bias=True)

        self.depthwise_conv32 = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, padding=2, stride=1, dilation=2, groups=c, bias=True)
        self.pointwise_conv32 = nn.Conv2d(in_channels=c, out_channels=c//2, kernel_size=1, bias=True)
